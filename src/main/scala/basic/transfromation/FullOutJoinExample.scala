...
object FullOutJoinExample {
  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder.appName("full out join example").master("local[2]").getOrCreate()
    ...
    val resultRDD = inputRDD1.fullOuterJoin(inputRDD2,2)
